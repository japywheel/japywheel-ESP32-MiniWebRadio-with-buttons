...
void loop() {
    ...
    JoyKey = 0;
    // 11 is right, 12 is left 13 is down,14 is up, 16 is OK
    ...
    switch(key) {
        ...
        case 11:    // ARROW RIGHT
                    ...
                    if(_state == STATIONSLIST) {    // next page
                        lst_RADIO.nextPage(); setTimeCounter(4);
                        return;
                    }
                    if(_state == PLAYER){
...
void tp_pressed(uint16_t x, uint16_t y) {
    // x y scaling correction for 2.4 inch screen
    /*
    x=int(x*1.3333333);
    y=int(y/1.3333333); 
    y=y-11;
...
void graphicObjects_OnClick(const char* name, uint8_t val) { // val = 0 --> is inactive
    ...
    if(_state == STATIONSLIST) {
        if( val && !strcmp(name, "lst_RADIO"))       {setTimeCounter(4); return;}
    }
    if(_state == PLAYER) {
    ...
    if(_state == AUDIOFILESLIST) {
        if( val && !strcmp(name, "lst_PLAYER"))      {setTimeCounter(4); return;}
    }
    if(_state == DLNA) {
